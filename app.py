...
with tab4:
    ...
    st.caption(
        "The 12 scenarios in the previous tab were generated by Phi-3-mini (an open-source LLM) "
        "as a one-time offline step, since running the full model live isn't feasible on free hosting. "
        "This tab lets you build a custom scenario yourself and see its effect on each company's DCF "
        "valuation in real time, using the same underlying model the LLM's scenarios were fed into."
    )
